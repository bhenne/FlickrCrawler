...
def check_personalized_urls_table(dbconn):
    ...
    try:
      ...
      if str(e).startswith('no such table'):
          dbcursor.execute('CREATE TABLE personalized_urls (purl VARCHAR(128), mobile BOOLEAN, status INT DEFAULT 0, PRIMARY KEY (purl) ON CONFLICT IGNORE)')
    dbcursor.close()
...
def personalized_urls_from_web(dbconn, url_list, mobile=False, crawls=1, wait_interval=[30,180]):
    ...
    for i in xrange(0, crawls):
        ...
        for p in resultset:
            dbcursor.execute('INSERT INTO personalized_urls VALUES (?,?,?)', [p,m,status])
        dbconn.commit()
        if i < crawls-1: 
...
def get_personalized_urls(dbconn, mobile=None, status=None, all_columns=False, limit=None):
    ...
    if all_columns == False:
        ...
        q = 'SELECT purl, mobile, status FROM personalized_urls WHERE 1=1'
