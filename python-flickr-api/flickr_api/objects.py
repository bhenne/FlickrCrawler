...
class FlickrObject(object):
    ...
    __display__ = [] # The attribute to display when the object is converted to a string
...
class Gallery(FlickrObject):
    ...
    def getInfo(self):
        def fromat_result(r,token = None):
            gallery = r["gallery"]
            ...
            gallery["primary_photo"] = Photo(id = pp_id, secret = pp_secret, server = pp_server, farm = pp_farm, token = token)
            return gallery
        return {},format_result
...
class Photo(FlickrObject):
    ...
    def getContext(self,**args):
        def format_result(r,token):
            Photo(token = token,**r["prevphoto"]),Photo(token = token,**r["nextphoto"])
        return args,format_result
    ...
    def getComments(self,**args):
        def format_result(r,token):
            ...
            for c in comments :
                ...
                comments_.append(Photoset.Comment(token = token,photo = self,**c))
            return comments_
        return args,format_result
